import numpy as np
#/home/kamron/.conda/envs/deepmd2/lib/python3.7/site-packages/dpdata/vasp/outcar.py
def system_info (lines, type_idx_zero = False) :
    atom_names = []
    atom_numbs = None
    nelm = 10    # ** was None  did add line before ions NELM = # but removed it now and will adjust it here
    for ii in lines: 
        if 'POTCAR' in ii : # was TITEL
            # get atom names from POTCAR info, tested only for PAW_PBE ...
            _ii=ii.split()[2]  # was 3
            if '_' in _ii:
...
def analyze_block(lines, ntot, nelm) :
    ...
    for idx,ii in enumerate(lines) :
        ...
        elif 'direct lattice vectors' in ii:  # ** was VOLUME AND BASIS
            for dd in range(3) :
                tmp_l = lines[idx+1+dd]     # ** 5 changed to 1
